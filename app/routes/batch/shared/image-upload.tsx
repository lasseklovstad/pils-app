        ...
        <Input
            onChange={async (event) => {
                ...
                if (files && files.length > 0) {
                    ...
                    const response = await fetch(href("/api/image-sas-token") + "?" + new URLSearchParams({ batchId: batchId.toString(), numberOfFiles: files.length.toString() }).toString())
                    const sasTokens: Awaited<ReturnType<typeof loader>> = await response.json()
