...
export const getBatchFiles = async (batchId: number) => {
  return await db
    ...
    .from(batchFiles)
    .where(and(eq(batchFiles.batchId, batchId), eq(batchFiles.isDeleted, false)));
};

export const getBatchFile = async (fileId: string) => {
  return (
    await db.select().from(batchFiles).where(and(eq(batchFiles.id, fileId), eq(batchFiles.isDeleted, false)))
  )[0];
};
...
export const deleteFile = async (fileId: string) => {
  ...
  await db.update(batchFiles).set({ isDeleted: true }).where(eq(batchFiles.id, fileId));
};
